                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
    TreeNode* smallestSingle = NULL;
    TreeNode* smallestMerge = NULL;
    if (! singleQueue.empty())
        smallestSingle = singleQueue.front();
    if (! singleQueue.empty())
        smallestMerge = mergeQueue.front();
    if (smallestSingle == NULL && smallestMerge != NULL) {
        mergeQueue.pop();
        return smallestMerge;
    } else if (smallestSingle != NULL && smallestMerge == NULL) {
        singleQueue.pop();
        return smallestSingle;
    } else if (smallestSingle != NULL && smallestMerge != NULL) {
        if (smallestSingle->freq < smallestMerge->freq) {
            singleQueue.pop();
            return smallestSingle;
        } else {
            mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while (! (singleQueue.empty() && mergeQueue.size() == 1)) {
         ...
         TreeNode* newNode = new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
         newNode->left = left;
         newNode->right = right;
