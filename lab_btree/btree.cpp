...
template <class K, class V>
V BTree<K, V>::printfind(const K& key) const
{
    return root == nullptr ? V() : printfind(root, key);
}

template <class K, class V>
V BTree<K, V>::printfind(const BTreeNode* subroot, const K& key) const
{
    ...
    if (first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == key)
     ...
     if (!subroot->is_leaf)
        return printfind(subroot->children[first_larger_idx], key);
