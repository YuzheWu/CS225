...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     int partition(vector<Point<Dim>>& points, int left, int right, int pivotIndex, int curDim);

     Point<Dim> quickSelect(vector<Point<Dim>>& points, int left, int right, int k, int curDim);

     Point<Dim> findNearestNeighbor_(const Point<Dim>& query, int currDim, KDTreeNode* currNode) const;

     KDTreeNode* buildSubTree(vector<Point<Dim>>& points, int left, int right, int curDim);
};
